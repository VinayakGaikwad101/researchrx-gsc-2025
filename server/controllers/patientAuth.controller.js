...
export const patientRegister = async (req, res) => {
  try {
    const { firstName, lastName, email, password, phoneNo, dob, gender, role } =
      req.body;
    ...
    if (userExist) {
      if (userExist.role === "Researcher") {
        return res.status(400).json({
          message: "Email already exists as an researcher",
          success: false,
        });
      }
    ...
    console.error("Error in patientRegister controller: ", error);
...
export const verifyEmail = async (req, res) => {
  try {
    ...
    console.error("Error in patient verify email controller: ", error);
...
export const regenerateOTP = async (req, res) => {
  try {
    ...
    console.error("Error in patient regenerate OTP controller: ", error);
    res.status(500).json({
...
export const patientLogin = async (req, res) => {
  try {
    ...
    if (userExist.role === "Researcher") {
      console.error("User is a researcher");
      return res.status(403).json({
        message: "Access denied. Only patients are allowed to login",
        success: false,
      });
    }
    ...
    console.error("Error in patient login controller: ", error);
    res.status(500).json({ message: "Server error", success: false });
  }
};
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    await sendPatientPasswordResetEmail(userExist.email, resetToken);
    ...
    console.error("Error in patient forgot password controller: ", error);
    res.status(500).json({ message: "Server error", success: false });
  }
};
